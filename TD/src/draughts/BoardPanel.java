...
public class BoardPanel extends JPanel {
    ...
    public void deleteIfBeaten(Checker first, Checker second) {
        ...
        if (second.getKind() == Sort.queenBlack || second.getKind() == Sort.queenWhite) {
            if ((first.getI() < second.getI() && first.getJ() < second.getJ()) || (first.getI() > second.getI() && first.getJ() > second.getJ())) {
                for (int p = 1; p < Math.abs(second.getI() - first.getI()); p++) {
                    if (checkersArray[iMax - p][jMax - p].getKind() == opponent1 || (checkersArray[iMax - p][jMax - p].getKind() == opponent2)) {
                ...
                for (int p = 1; p <= Math.abs(first.getI() - second.getI()); p++) {
                    if (checkersArray[iMax - p][jMin + p].getKind() == opponent1 || (checkersArray[iMax - p][jMin + p].getKind() == opponent2)) {
            ...
            if (checkersArray[iMax - 1][jMax - 1].getKind() == Sort.fullWhite || checkersArray[iMax - 1][jMax - 1].getKind() == Sort.queenWhite) {
                whites--;
            } else {
                blacks--;
            }
        ...
        if (deleted) {
            if (blacks == 0) {
                JOptionPane.showMessageDialog(null, "Koniec gry. Zwyciężył kolor biały.", "Koniec gry", JOptionPane.INFORMATION_MESSAGE);
            } else if (whites == 0) {
                JOptionPane.showMessageDialog(null, "Koniec gry. Zwyciężył kolor czarny.", "Koniec gry", JOptionPane.INFORMATION_MESSAGE);
            }
    ...
    private ArrayList<Checker> checkBeating(Checker jb) {
        ...
        if (jb.getKind() == Sort.queenWhite || jb.getKind() == Sort.queenBlack) {
            for (int p = 2; p < 10; p++) {
                if (i + p < 10 && j + p < 10) {
                    if (getCheckersArray()[i + p - 1][j + p - 1].getKind() == supporter1 || getCheckersArray()[i + p - 1][j + p - 1].getKind() == supporter2) {
                        break;
                    }
                    if (getCheckersArray()[i + p - 1][j + p - 1].getKind() == opponent1 || getCheckersArray()[i + p - 1][j + p - 1].getKind() == opponent2) {
                        if (getCheckersArray()[i + p][j + p].getKind() != Sort.blankBlack) {
                            break;
                        }
                        beatingMoves.add(getCheckersArray()[i + p][j + p]);
                        int q = 1;
                        while (i + p + q < 10 && j + p + q < 10 && getCheckersArray()[i + p + q][j + p + q].getKind() == Sort.blankBlack) {
            ...
            for (int p = 2; p < 10; p++) {
                if (i + p < 10 && j - p >= 0) {
                    ...
                    if (getCheckersArray()[i + p - 1][j - p + 1].getKind() == opponent1 || getCheckersArray()[i + p - 1][j - p + 1].getKind() == opponent2) {
                        ...
                        beatingMoves.add(getCheckersArray()[i + p][j - p]);
                        int q = 1;
                        while (i + p + q < 10 && j - p - q >= 0 && getCheckersArray()[i + p + q][j - p - q].getKind() == Sort.blankBlack) {
                            beatingMoves.add(getCheckersArray()[i + p + q][j - p - q]);
                            q++;
                        }
                        break;
            ...
            for (int p = 2; p < 10; p++) {
                if (i - p >= 0 && j + p < 10) {
                    ...
                    if (getCheckersArray()[i - p + 1][j + p - 1].getKind() == opponent1 || getCheckersArray()[i - p + 1][j + p - 1].getKind() == opponent2) {
                        ...
                        beatingMoves.add(getCheckersArray()[i - p][j + p]);
                        int q = 1;
                        while (i - p - q >= 0 && j + p + q < 10 && getCheckersArray()[i - p - q][j + p + q].getKind() == Sort.blankBlack) {
                            beatingMoves.add(getCheckersArray()[i - p - q][j + p + q]);
                            q++;
                        }
                        break;
            ...
            for (int p = 2; p < 10; p++) {
                if (i - p >= 0 && j - p >= 0) {
                    ...
                    if (getCheckersArray()[i - p + 1][j - p + 1].getKind() == opponent1 || getCheckersArray()[i - p + 1][j - p + 1].getKind() == opponent2) {
                        ...
                        beatingMoves.add(getCheckersArray()[i - p][j - p]);
                        int q = 1;
                        while (i - p - q >= 0 && j - p - q >= 0 && getCheckersArray()[i - p - q][j - p - q].getKind() == Sort.blankBlack) {
                            beatingMoves.add(getCheckersArray()[i - p - q][j - p - q]);
                            q++;
                        }
                        break;
            ...
            if (i - 2 >= 0 && j - 2 >= 0) {
                if ((getCheckersArray()[i - 1][j - 1].getKind() == opponent1 || getCheckersArray()[i - 1][j - 1].getKind() == opponent2) && getCheckersArray()[i - 2][j - 2].getKind() == Sort.blankBlack) {
            ...
            for (int j = 0; j < 10; j++) {
                if ((i + j) % 2 != 0) {
                    if (parentFrame.getMyPlayer().getMAuthor() == Author.owner) {
                        if ((getCheckersArray()[i][j].getKind() == Sort.fullWhite || getCheckersArray()[i][j].getKind() == Sort.queenWhite) && checkBeating(getCheckersArray()[i][j]).size() != 0) {
    ...
    public class ClickActionListener implements ActionListener {
        ...
        /**
         * Sprawdza czy użytkownik chce wykonać dozwolony ruch. Po wykonaniu ruchu decyduje czy użytkownik zakończył sekwencję ruchów oraz jeśli wystąpiły bicia usuwa zbite pionki z planszy. Wysyła informację o ruchu przeciwnikowi.
         * @param e Zdarzenie.
        ...
        public void actionPerformed(ActionEvent e) {
            if (parentFrame.isGameIsOn() && parentFrame.isYourTurn()) {
                ...
                } else if (jb.getKind() == Sort.blankBlack && chosen == true && (freeMoves.contains(jb) || toBeatList.contains(jb))) {
    ...
    public class RightClickListener extends MouseAdapter {
        ...
        public void mousePressed(MouseEvent e) {
            if (e.getButton() == MouseEvent.BUTTON3 && parentFrame.isGameIsOn() && parentFrame.isYourTurn() && chosenLast != null) {
                chosen = false;
                chosenLast.setBorder(null);
            }
        }
     ...
     * @param parentFrame Okno, które zawiera planszę.
     */
